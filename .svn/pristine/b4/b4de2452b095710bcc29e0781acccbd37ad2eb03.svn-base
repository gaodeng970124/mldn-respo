...
public class ParameterUtil {
	 ...
	 * @param request 请求对象
	 * @param tempDir 临时的保存目录
	 * @param parentFile 设置一个相对的父目录的路径
	 */
	 ...
	 * @param request 请求对象
	 * @param tempDir 临时的保存目录
	 * @param sizeMax 最大的上传文件
	 * @param fileSizeMax 单个文件的大小控制
	 * @param charset 编码
	 */
	public ParameterUtil(HttpServletRequest request, String tempDir, long sizeMax, long fileSizeMax, String charset) {
		...
		if (this.request.getContentType() != null
				&& this.request.getContentType()
					.startsWith("multipart/form-data")) { // 必须在保证有上传表单封装的前提下才可以使用FileUpload
	 ...
	 * @param fileSizeMax 单个文件的大小控制
	 * @param charset 编码
	 * @param parentFile 文件保存父目录
	 */
	public ParameterUtil(HttpServletRequest request, String tempDir, long sizeMax, long fileSizeMax, String charset,String parentFile) {
		this.request = request ; // 保存request对象
		this.charset = charset ; // 保存参数的接收编码
